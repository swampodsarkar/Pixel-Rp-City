...
const Sedan = ({ color }: { color: string }) => (
  <group>
    ...
    <mesh position={[0, 0.75, -1.5]} rotation={[-0.4, 0, 0]}>
      <planeGeometry args={[1.4, 0.7]} />
      <meshStandardMaterial color="#111827" roughness={0.1} metalness={0.8} />
    </mesh>
    <Wheels />
...
const SportsCar = ({ color }: { color: string }) => (
  <group>
    ...
    <mesh position={[0, 0.5, -1.4]} rotation={[-0.3, 0, 0]}>
      <planeGeometry args={[1.8, 0.5]} />
      <meshStandardMaterial color="#111827" roughness={0.1} metalness={0.8} />
    </mesh>
...
const Ambulance = () => (
  <group>
    ...
    <mesh position={[1.11, 0.3, 0.5]} rotation={[0, Math.PI / 2, 0]}>
...
const Truck = ({ color }: { color: string }) => (
  <group>
    <mesh castShadow position={[0, 0.4, 1.2]}>
    ...
    <mesh position={[0, 0.9, 2]} rotation={[0.4, 0, 0]}>
  ...
  <>
    {[-1.05, 1.05].map(x =>
      [-wheelOffset, wheelOffset].map(z => (
        <mesh key={`w-${x}-${z}`} castShadow rotation={[0, 0, Math.PI / 2]} position={[x, wheelRadius, z]}>
...
export const RealisticCar = ({ color = "#ef4444", type = "sedan" }: { color?: string; type?: string }) => {
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]} scale={1.5}>
      ...
      <pointLight position={[1, 0.3, 3]} intensity={1.5} distance={20} color="#fef08a" />
      <pointLight position={[-1, 0.3, 3]} intensity={1.5} distance={20} color="#fef08a" />
